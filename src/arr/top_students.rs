...
impl Solution {
    pub fn top_students(positive_feedback: Vec<String>, negative_feedback: Vec<String>,
                        report: Vec<String>, student_id: Vec<i32>, k: i32) -> Vec<i32> {
        ...
        for i in 0..n {
            ...
            for x in report[i].split(' ') {
                if positive.contains(x) {
                    score += 3;
                } else if negative.contains(x) {
                    score -= 1;
                }
            }
            scores[i] = (score, student_id[i]);
        }
        scores.sort_unstable_by(|&s1, &s2| {
            match s2.0.cmp(&s1.0) {
